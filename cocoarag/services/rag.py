...
class GetSimilarChunksService:
    def __call__(
        self, user_id: str, group_id: str, query: QueryModel, filters: FiltersModel
    ) -> list[ChunkModel]:
...
class BuildContexPromptService:
    def __call__(self, history: list, chunks: list[ChunkModel]) -> str:
        ...
        context_payload = {
            "document": [chunk.content.decode("utf-8") for chunk in chunks]
        }
        context_payload = json.dumps(context_payload)

        prompt: str = rag_template_english_v1.format(
            conversation_history=conversation_history, context_payload=context_payload
        )
...
class GenerateAnswerService:
    ...
    def __init__(self, config_path="../configs/credits.yml"):
        ...
        self.client = OpenAI(api_key=self.config.retrieve_model.open_ai.token)
...
class QueryRAGSystemService:
    def __call__(
        ...
        if filters.content == {}:
            filters = FiltersModel(content={"user_id": {"$in": [user_id]}})

        chunks: list[ChunkModel] = service(
            user_id=user_id, group_id=group_id, query=query, filters=filters
        )
        ...
        generation_result: str = generation_service(
            context_prompt=context_prompt, user_prompt=query.content
        )
...
if __name__ == "__main__":
    ...
    def upload_document(user_id, group_id):
        ...
        service(user_id=user_id, user_group=group_id, document=document)
    ...
    def save_conversation_info(user_id, conversation_id):
        history = [
            ...
            {
                "role": "assistant",
                "content": "The population of Paris is \
             around 2.1 million.",
            },
        ]
    ...
    def check_conversations_work():
        ...
        answer = rag_query(user_id, user_group, conversation_id, query, filters)
